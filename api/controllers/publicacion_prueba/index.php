<?php
    include '../cors.php';
    require '../../models/publicacion_prueba.php';
    require '../autenticated.php';
    $datos = json_decode(file_get_contents("php://input"),TRUE); 
    
    if(isset($_GET['enviar_correo_masivo'])) {
        $data = validarPostData('data');
        if(!isset($data)){
            echo json_encode(array('status'=>'fail', 'message'=>'no data', 'data'=> null));
            return 0;
        }
        $vender = new Publicacion_prueba();
        $res = $vender->enviarCorreoPublicacionMasiva( $data );
        echo json_encode($res);
        return 0; 
    }

    if(isset($_GET['publicar_masivo'])) {
        $data = validarPostData('data');
        if(!isset($data)){
            echo json_encode(array('status'=>'fail', 'message'=>'no data', 'data'=> null));
            return 0;
        ...
        $vender = new Publicacion_prueba();
        $res = $vender->publicarMasivo($data); 
        echo json_encode($res);
        return 0; 
    }
